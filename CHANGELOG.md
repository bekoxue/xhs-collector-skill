...
- 修复本地页面生成的令牌被默认发往生产服务器而返回 `INVALID_TOKEN`
- 本地 `configure` 成功后持久化本机测试地址，并支持显式切回生产环境
- 本地地址严格限制为 `localhost`、`127.0.0.1` 或 `::1` 的 HTTP 根地址

## 0.1.8 - 2026-07-19

- enrich 在计费请求前原子保存操作幂等键；响应丢失时可用同一 `resume_hint` 找回原任务，避免手动重跑产生重复任务和扣费
- enrich 续采剩余 ID 时自动换用新的幂等键，确保下一批任务可正常创建
