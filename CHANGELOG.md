...
- 每次初始采集生成带 UUID 的独立续采文件，避免同关键词、同博主及 replies / enrich 任务覆盖彼此的游标、会话与去重状态
- 使用 `--resume-file` 续采时原子更新传入的同一文件；替换失败会保留旧断点并明确禁止重复执行旧提示或从头重采

## 0.1.6 - 2026-07-17

- 输出文件名增加微秒时间戳与 UUID，并使用独占模式创建 JSON/CSV；并发或同秒运行相同任务不再静默覆盖已付费结果，即使路径碰撞也会保留原文件
- JSON、CSV、stdout、请求体、断点指纹及断点文件安全处理未配对 Unicode；底层编码异常继续使用结构化防重复扣费提示，不再泄漏 traceback
